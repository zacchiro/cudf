...
module Make (Extra : Cudf.Extra) = struct

  module Cudf = Cudf.Make(Extra)
  open Cudf
  open Cudf_types
...
(* !!!!!!! XXX !!!!!!!!!! *)
(* not sure how to write something like
 * open Cudf.Make(Cudf.ExtraDefault)
 * *)
open Cudf
open Cudf_types
...
let pp = pp_property fmt in
  ...
  if pkg.keep <> default_package.keep then
    Option.may (fun k -> pp ("Keep", string_of_keep k)) pkg.keep;
  List.iter (fun (k,v) -> pp (k, ExtraDefault.to_string v)) pkg.extra
